        ...
        for (const auto *collision : gas->collisions[(uint8_t)CollisionType::ionization])
        {
            ...
            case IonizationOperatorType::sdcs:
                ...
                for (uint32_t k = 0; k < grid.cellNumber; ++k)
                {
                    ...
                    if (k > numThreshold)
                    {
                        const auto half = (k + 1 - numThreshold) / 2;
                    ...
                    if (end < grid.cellNumber)
                    {
                        for (uint32_t i = end - 1; i < grid.cellNumber; ++i)
                        {
                            ionizationMatrix(k, i) +=
                                density * grid.step * grid.getCell(i) * cellCrossSection[i] /
                                (std::atan((grid.getCell(i) - threshold) / (2 * W)) *
...
void ElectronKinetics::solveSpatialGrowthMatrix()
{
    ...
    Vector tempVector = grid.getCells().array() / cellTotalCrossSection.array();

    Vector D0 = tempVector / 3.,
    ...
    {
        if (j != 0)
            U0sup[j] = EoN / (6 * grid.step) * tempVector[j - 1];

        if (j != grid.cellNumber - 1)
            U0inf[j] = -EoN / (6 * grid.step) * tempVector[j + 1];
    }
    ...
    while (!hasConverged)
    {
        g_fieldSpatialGrowth = (alphaRedEffNew * EoN / 6) * grid.getNodes().array() / mixture.totalCrossSection.array();
        ...
        for (uint32_t k = 0; k < grid.cellNumber; ++k)
        {
            ...
            if (k > 0)
                ionSpatialGrowthU(k, k - 1) = alphaRedEffNew * U0inf[k];

            if (k < grid.cellNumber - 1)
                ionSpatialGrowthU(k, k + 1) = alphaRedEffNew * U0sup[k];
        }
        ...
        for (uint32_t k = 0; k < grid.cellNumber; ++k)
        {
            ...
            if (k > 0)
                ionSpatialGrowthU(k, k - 1) = 1.e20 * (fieldMatrixSpatGrowth(k, k - 1) + ionSpatialGrowthU(k, k - 1));
