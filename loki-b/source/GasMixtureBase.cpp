...
void GasMixtureBase::loadStateProperties(const StatePropertiesSetup &setup,
                                 const WorkingConditions *workingConditions)
{
...
void GasMixtureBase::loadStateProperties(const json_type &cnf,
                                 const WorkingConditions *workingConditions)
{
...
void GasMixtureBase::loadGasProperties(const GasPropertiesSetup &setup)
{
    ...
    for (const auto &fractionStr : setup.fraction) {
        ...
        auto it = std::find_if(m_gases.begin(), m_gases.end(), [&name](std::unique_ptr<GasBase>& gas) {
