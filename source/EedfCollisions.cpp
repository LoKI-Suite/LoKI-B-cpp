                                                      ...
                                                      const double OPBParameter) const
{
    ...
    if (type() == CollisionType::ionization)
    {
        ...
        else if (ionizationOperatorType == IonizationOperatorType::sdcs)
        {
            ...
            {
                collPower.forward = -SI::gamma * getTarget()->delta() * grid->getCell(lmin) * grid->duCell(lmin) *
                                    eedf.cwiseProduct(grid->getCells().cwiseProduct(grid->duCells() * TICS)).sum();
            }
        }
    }
...
RateCoefficient EedfCollision::evaluateRateCoefficient(const Vector &eedf)
{
    ...
    {
        m_ineRateCoeff = SI::gamma * cellCrossSection.cwiseProduct(grid->getCells().tail(nCells - lmin)
                             .cwiseProduct(grid->duCells().tail(nCells - lmin))).dot(eedf.tail(nCells - lmin));
    }
