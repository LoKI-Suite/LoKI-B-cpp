...
\begin{framed}
TODO/IDEA: is it useful to use this constraint to eliminate one the derivatives,
and make some general progress, instead of assuming already at this point that one
of them is zero (next sections)? I cannot imagine that that has not already been
tried before.
...
\end{align}
But using this is must be shown that it does not matter which growth model is assumed when
$\MEAN{\nu_{eff}}=0$, so we can arbitrarily set $\beta=1$ in that case.

What impact would such more general approach have on fluid models that
...
The expression for $\Gamma_z$ can be used to evaluate the power that is absorbed
by the field directly, the result can be written as
\begin{equation}
...
(item 19.) for the spatial growth case. Alternatively, the term from the field operator
...
spatial inhomogeneities. Then the energy balance takes the form

Substitution of this result and equation \eqref{eq:Gamma_u-sst} in the energy balance
and division by $Nn_e$ result in
\begin{equation}
	\frac{\alpha_{\mbox{eff}}}{N}\MEAN{u}\left(\frac{E}{N}(\mu_u E) - \frac{\alpha_{\mbox{eff}}}{N}(D_uN)\right)
	- \frac{q_e}{e}\left((\mu_eN) \frac{E}{N} - (D_eN)\frac{\alpha_{\mbox{eff}}}{N}\right)\frac{E}{N}
	= \int\limits_0^\infty u \tilde{C}^0(u)\dd{u}.
\end{equation}
or
\begin{equation}
	\frac{\alpha_{\mbox{eff}}}{N}\left(\MEAN{u}\left[\frac{E}{N}(\mu_u E) - \frac{\alpha_{\mbox{eff}}}{N}(D_uN)\right]
	+ \frac{q_e}{e} (D_eN)\frac{E}{N}\right)
	- \frac{q_e}{e}\left(\frac{E}{N}\right)^2(\mu_eN)
	= \int\limits_0^\infty u \tilde{C}^0(u)\dd{u}.
\end{equation}
TODO: repeat the transformations needed to get the BOLSIG+ expressions, check signs etc.
The first term now corresponds to the `growth power' (item 22) in \cite[p. 19]{BolsigManual2016}.
So, the additional term on the left-hand side should either be made part of the field operator
(that appears to be the most `physical' form, since it corresponds to the absorption $P_E=J_eE$),
{\em or} be made part of the growth power --- but not both.
\begin{framed}
...
\end{framed}
NOTE: the expression after substitution of the solution of $\alpha_{\mbox{eff}}$ above is
only valid for $E\neq 0$. This is the last form of
\begin{equation}
	\MEAN{v_z}
	=\frac{\Gamma_z}{n_e}
...
\end{equation}
Also the case $E=0$ should be captured here.
