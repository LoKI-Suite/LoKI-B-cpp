...
is consistent with the discretization of $\dd{H_{E,x}^0(u)}/\d{u}$ that is
...
\[
	H_x(u) = -h_x(u)\left(c_x(u) f^0(u) + d_x(u) f^0(u)\right)
	       = -h_x(u)d_x(u) f^0(u) - h_xc_x(u) f^0(u),
\]
...
NOTE: Here we have followed the LoKI-B docs, which call the convection coefficient
$d_x$ and the diffusion coefficient $c_x$ --- which is somewhat unfortunate ---
also see section \ref{sec:dH/du-disc}.
...
That can be obtained by multiplying this equation with $u$ and integrating over the energy grid;
the result is
\begin{equation}
	\sum\limits_{x=el,car,ee,E_g}\int\limits_0^\infty u\DERIV{H_x}{u}\dd{u}
	=
...
\begin{equation}
	\MEAN{\DERIV{H_x}{u}}_k
	= \frac{1}{(\Delta u)_k}\int\limits_{\mhalf{k}}^{\phalf{k}}\DERIV{H_x}{u}\dd{u}
	= \frac{H_{x,\phalf{k}}-H_{x,\mhalf{k}}}{(\Delta u)_k}.
...
The result of that
discretization process is that every term is cast as the product of a matrix
and the vector of cell values of the EEDF. The discrete form of equation
\eqref{eq:EBE-final-avg} then takes the form\footnote{In the LoKI-B code, all
terms are brought to the {\em right}-hand side; the resulting matrices then
have an additional minus sign.}
\begin{equation}
	\sum\limits_{x=el,car,ee,E_g}\bmat{M}_x\bvec{f}
	=
	-\sum\limits_{x=inel,ion,att}\bmat{M}_x\bvec{f}
	-\bmat{M}_g\bvec{f}
	\iff
	\sum\limits_{\substack{x=el,car,ee,E_g,\\inel,ion,att,g}}\bmat{M}_x\bvec{f}
	=\bvec{0},
\end{equation}
or
\begin{equation}
...
where
\begin{equation}
	\bmat{M}(\bvec{f}) = \sum\limits_{\substack{x=el,car,ee,E_g,\\inel,ion,att,g}}\bmat{M}_x.
\end{equation}
The power balance has been presented in equation \eqref{eq:EBE-power-balance}.
Every term ${\cal T}_x(\bvec{f})[\bvec{f}]$ in the Boltzmann equation can be discretized as
\begin{equation}
	\int\limits_0^\infty u{\cal T}_x(\bvec{f})[\bvec{f}]\dd{u}
	\doteq
	\sum\limits_{i=0}^{{\cal N}-1}u_i [{\cal T}_x(\bvec{f})[\bvec{f}]]_i (\Delta u)_f
	\doteq
...
\chapter{General Notes on Discretization}

TODO: the matrices that are presented in this chapter are based on the integral
form of the Boltzmann equation (per cell). Elsewhere, a division by $(\Delta u)_P$
is done and the terms represent {\em average} values (of $\dd{H}/\dd{u}$, for
example). Also in \SRC{ideas/example_cd_discretizer.cpp} we have the latter
form. Let us change the text and also use the `average' formulation in the
present chapter instead, when merging with the previous one.

\label{ch:disc-general}
...
\[
	...
	\tilde{R}^0_g(u).
\]
The flux densities that appear on the left-hand side of this equation are of the
form
\begin{equation}
	H_x(u) = C_x(u)f - D_x(u)\DERIV{f}{u}.
	\label{eq:disc:cd-flux}
\end{equation}
...
\begin{equation}
	...
	\label{eq:disc:be}
\end{equation}
where $S$ represents the terms that appear on the right-hand side of the
Boltzmann equation (the ones that cannot be written in flux-divergence form),
...
We will now discuss how the Boltzmann equation can be discretized and solved
using the finitite volume method. We will then discuss how the individual
terms can be evaluated in a consistent way.

For a given finite volume $P$, the Boltzmann equation \eqref{eq:disc:be}
can be integrated to
\begin{equation}
	H(u_e) - H(u_w) = \int\limits_{u_w}^{u_e}S(u)\dd{u},
	\label{eq:disc:be-P-int}
\end{equation}
where $w$ and $e$ are the faces at the lower and upper boundary of finite
volume $P$.  Note that this equation is still exact. The first approximation
is to replace the source $S(u)$ with its value in the point $P$ in the center
of the finite volume, resulting in the equation
\begin{equation}
	H(u_e) - H(u_w)
	= S(u_P)(u_e-u_w)
	= S(u_P)(\Delta u)_P
	...
	\label{eq:disc:be-P}
\end{equation}
What remains is the derivation of a suitable expression for the fluxes at the
faces $e$ and $w$ (east and west). This `flux approximation' is the key element
in the development of a finite volume discretization scheme.

TODO: $f$ is used in two different ways in the text below: face and field.
We will be looking for flux expressions at faces that depend only on the
values in the cells adjacent to the faces. For an {\em internal} face $f$
we will call the cells at the lower and upper energy sides $L$ and $U$,
respecively. We will demonstrate later that the resulting expression can be
written as
\begin{equation}
	H(u_f) = B_ff_L - A_ff_H,
	\label{eq:disc:H(B,A)}
...
equation \eqref{eq:disc:be-P} yields, for any internal cell $P$,
\begin{equation}
	{\cal T}_P
...
\end{equation}
For a boundary face, equation \eqref{eq:disc:H(B,A)} cannot be used umodified:
on the low-energy grid boundary cell $L$ is missing, on the high-energy
...
equation a suitable condition is that the boundary flux is zero at both ends
of the grid. As an example, on the western boundary the discrete equation
becomes
\begin{equation}
	{\cal T}_{P=0}
	= (B_ef_P - A_ef_E) - 0
...
\end{align}
Following Patankar \cite{Patankar1980}, this can be written as
\begin{align}
	A_f &= \frac{D_f}{(\Delta u)_f}\left(1-P_fc_f    \right), \label{eq:disc:A_f-cd} \\
	B_f &= \frac{D_f}{(\Delta u)_f}\left(1+P_f(1-c_f)\right). \label{eq:disc:B_f-cd}
\end{align}
Here we have introduced the {\em grid P\'eclet number} $\Pe_f$, that will
come back in the further discussions of the discretization schemes, as
\begin{equation}
	\Pe_f = \frac{C_x(\Delta u)_f}{D_f}.
	\label{eq:Peclet}
\end{equation}
In the special case of a uniform mesh, with $c_f=\frac{1}{2}$, these expressions for the
coefficients reduce to
\begin{align}
	A_f &= \frac{D_f}{(\Delta u)_f}\left(1-\frac{\Pe_f}{2}\right), \label{eq:disc:A_f-cd-uniform} \\
	B_f &= \frac{D_f}{(\Delta u)_f}\left(1+\frac{\Pe_f}{2}\right). \label{eq:disc:B_f-cd-uniform}
\end{align}

Equation \eqref{eq:H_f:cd} demonstrates that the expression for the flux is linear
in the coefficients $C_f$ and $D_f$. As a result, the discretization matrix
\eqref{eq:disc:H-term} is the sum of the discretization matrices that can be
constructed for the individual flux contributions $H_x$. We will soon see that
this is not true for other discretization schemes, such as the exponential
scheme, which we will discuss next.

\subsection{The Scharfetter-Gummal (Exponential) Scheme}

More sophisticated schemes are obtained by solving a simplified form of
equations \eqref{eq:disc:be} and \eqref{eq:disc:cd-flux-total} for $f(u)$
...
\end{equation}
and the local value of the P\'eclet number is defined by
\begin{equation}
