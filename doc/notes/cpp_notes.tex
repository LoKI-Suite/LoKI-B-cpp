...
faces $e$ and $w$ (east and west). This `flux approximation' is the key element in the
derivation of a finite volume discretization scheme.
...
\end{equation}
Using the function ${\cal B}(\Pe)$ avoids the singularity for $\Pe=0$. Note that for
$\Pe\neq 0$ we have
\begin{align}
	f(u(c))
...
\end{align}
The last form also gives the correct result for $\Pe=0$.
This is again of the form \eqref{eq:disc:H(B,A)}, but this time with
\begin{align}
	B_f &= \frac{D}{(\Delta u)_f}\left({\cal B}(\Pe)+\Pe\right), \label{eq:disc:B_f-exp} \\
	A_f &= \frac{D}{(\Delta u)_f}{\cal B}(\Pe). \label{eq:disc:A_f-exp}
\end{align}
Please note that the flux \eqref{eq:disc:H(C,D,P)} does not depend on $u$ (or, equivalently, $c(u)$),
...
Grouping the terms depending on $f_L$ and $f_H$, equation \eqref{eq:disc:Hx(Cx,Dx,P)} can be written as
\begin{equation}
	H_x = B_xf_L - A_xf_H,
\end{equation}
with
\begin{align}
	...
	B_x &= A_x + C_x.
\end{align}
A few notes about the evlauation follow. Firstly, let us assume that $D_x\neq 0$, so the
