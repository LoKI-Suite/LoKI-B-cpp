...
is Rockwood's 1973 paper \cite{Rockwood1973}. Appendix \ref{sec:Rockwood1973EE} contains a
derivation of LoKI's expressions, starting from the presentation of Rockwood.
\end{framed}
...
\end{align}
Three more modifications are made (only sketched in the manual). A reference to
Rockwood \cite{Rockwood1973} is provided for these steps:
...
\begin{itemize}
  ...
  \item It is better to define these matrices and vectors without the leading
	factor $g_{ee}$. Then the matrices $\bmat{a}$ and $\bmat{b}$ depend
	only on the grid properties, the vectors can be calculated by taking
	the matric product with the EEDF, and the factor $g_{ee}$ can be
	applied separately when these are used. This prevents unnecessary
	recalculations.
	It appears that this is also how things are implemented in the
	MATLAB code (version 2.2.0, \SRC{ElectronKinetics.m} line 1359).
	In the MATLAB code (line 1387) {\em and} in the C++ code
	(\SRC{source/Operators.cpp}) we apply a factor \SRC{g_ee/grid.du()}
	to the {\em vectors}.
	TODO: Make sure to document the state of affairs carefully, and ensure
	that the docs and the code are in sync.
\end{itemize}
\end{framed}
...
\begin{equation}
  J_{ee}(u) = 3\alpha_{eV} n_e^2 \left[-I(u)f_\epsilon - J(u)\PDEV{f_u}{u}\right],
\end{equation}
...
\begin{align}
	...
	&= -\frac{g_{ee}}{\Delta u}\left(
		-\frac{2}{3}u^{3/2}\frac{<1/\sqrt{u}>}{\Delta u} + \int\limits_0^{u_{\phalf{n}}} f(u')\left[\sqrt{u'} + \frac{2}{3\Delta u}(u^{3/2}-(u')^{3/2})\right]\dd{u'}
		\right),
		\\
	B_n &= +\frac{g_{ee}}{\Delta u}\left(\frac{I(u_{\mhalf{n}})}{2} + \frac{J(u_{\mhalf{n}})}{\Delta u}\right)
		\nonumber \\
	&= -\frac{g_{ee}}{\Delta u}\left(
		+\frac{2}{3}u^{3/2}\frac{<1/\sqrt{u}>}{\Delta u} + \int\limits_0^{u_{\mhalf{n}}} f(u')\left[\sqrt{u'} - \frac{2}{3\Delta u}(u^{3/2}-(u')^{3/2})\right]\dd{u'}
		\right).
\end{align}
