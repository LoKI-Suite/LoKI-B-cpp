...
\begin{align}
	I(u) &= \int\limits_0^u f(u')\sqrt{u'}\dd{u'}, \\
	J(u) &= \frac{2}{3}\left(
...
\begin{align}
	...
	b_{ij} &= g_{ee}\begin{cases}
		...
		\end{cases}. \\
\end{align}
TODO: discuss differences with the presentation in the manual.
TODO: present the modifications to ensure the correct behaviour (no electrons created, the operator must
also be zero for a Maxwellian $f$) In particular, find a reference for the modification $a_{ij}=\sqrt{a_{ij}a_{j-1,i+1}}$:
I cannot find this in Rockwood, which is cited in the manual.

\begin{framed}
Code observations:
\begin{itemize}
	\item In the code (MATLAB and C++) the name \SRC{alpha} is used instead of \SRC{g_ee}
		(see \SRC{Boltzmann.m} line 1385 in version 2.2.0; the same constant appears
		a second time on line 2020, but this time the name \SRC{eeConstant} is used
		instead of \SRC{alpha}.)
	\item 	The usage of the name \SRC{alpha} probably has its roots in the paper of Rockwood
		\cite[below eq. B1]{Rockwood1973}, but a direct comparison of this coefficient and
		the equations in which it appears is complicated because of Rockwood's usage of the
		Gaussian-CGS unit system (?), because he does {\em not} express energies in $eV$ and
		because he considers an equation for the distribution function with the factor $\sqrt{u}$
		included.
		The result is that factors $e$ and $4\pi\epsilon_0$ are different here and there.
		Hagelaar's $a$ \cite[eq. 37]{Hagelaar2005}
		The SI expression is obtained by the Gaussian-CGS to SI substitutions
		$e\rightarrow e/\sqrt{4\pi\epsilon}$ and by $\sqrt{2/m}\rightarrow \sqrt{2e/m}=\gamma$
		in Rockwood's $\alpha$:
		\[
			\alpha_{\mbox{rockwood}} = \frac{2}{3}\pi e^4\sqrt{2/m}\ln\Lambda_D
			\rightarrow
			\frac{2}{3}\pi (e/\sqrt{4\pi\epsilon_0})^4\sqrt{2e/m}\ln\Lambda_D
			=
			\frac{1}{24}e^4/(\pi\epsilon_0^2)\gamma\ln\Lambda_D.
		\]
		BUG: WE SHOULD HAVE $e^2$ instead of $e^4$. There must be more conversion confusion.
		Another factor 3 is produced because the functions $I(u)$ and $J(u)$ are used in LoKI-B,
		rather than the functions $P(u)$ and $Q(u)$ that appear in \cite[below eq. B5]{Rockwood1973}.
		When also adding the ionization degree $n_e/N$, and dividing by $\gamma$, the result
		is LoKI-B's $g_{ee}$.
	\item The SI-version of this $\alpha$ also appears in BOLSIG+ \cite[eq. 32-37]{Hagelaar2005};
		these equations are based on the non-divergence-flux versions of Rockwood's equations
		\cite[eq. B1--3]{Rockwood1973}.
	\item TODO: Replace alpha in the code with \SRC{g_ee} to match the documentation and avoid confusion
		with Rockwood's $\alpha$ and Hagelaar's $a$.
	\item In the code, the argument of the Coulomb logarithm is expanded as (NOTE: $T_e$ is in $\si{eV}$
		in LoKI-B):
		\begin{equation}
			\Lambda_c
		...
		\end{equation}
		TODO: improve the readability of these expressions by introducing \SRC{lambdaD}
		as an intermediate variable and using the first expression for $\Lambda_c$.
\end{itemize}
\end{framed}
