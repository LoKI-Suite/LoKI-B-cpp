...
\begin{align*}
	...
	\mu_uN &= \frac{\gamma}{\MEAN{u}}\int\limits_0^\infty u{\cal D}_{pt}^0(u)\DERIV{f}{u}\dd{u}.
\end{align*}
These can be expressed in terms of $f(u)$ through a partial integration. For example, for $\mu_eN$
we find that
\[
	\mu_eN
...
\begin{equation}
	C_k = -\gamma\begin{cases}
			...
			\frac{1}{2}({\cal D}^0_{1}-{\cal D}^0_{0}) & \text{otherwise}.
	\end{cases}
\end{equation}
...
TODO: the expression for $D_uN$. This is very simular: replace $\gamma -> \gamma/\MEAN{u}$ and $D^0_k->u_kD^0_k$.
