...
\begin{equation}
	H_{E,x}^0(u) = - \gamma{\cal D}_{x}^0(u)\PDEV{f^0_u}{u},
	\label{eq:H_E,x^0}
\end{equation}
so equation \eqref{eq:H_E,x} can be written as
\begin{equation}
	H_{E,x}(u) = \left(\frac{E}{N}\right)^2H_{E,x}^0(u).
\end{equation}
In terms of this quantity, \eqref{eq:mueN} takes the form
\begin{equation}
	\mu_eN = -\int\limits_0^\infty H_{E,x}^0(u) \dd{u}.
\end{equation}
This can be manipulated as follows:
\begin{equation}
	\mu_eN
	= -\int\limits_0^\infty H_{E,x}^0(u) \DERIV{u}{u} \dd{u}
	= -\int\limits_0^\infty \left(\DERIV{uH_{E,x}^0(u)}{u} - u\DERIV{H_{E,x}^0(u)}{u}\right)\dd{u}
	= +\int\limits_0^\infty u\DERIV{H_{E,x}^0(u)}{u}\dd{u}.
	\label{eq:mueN-alt}
\end{equation}
Here we have used the fact that $uH_{E,x}^0(u)$ vanishes for $u=0$ and for
$u\rightarrow\infty$. (TODO: announce that the discrete form of this equation
is consistent with the discretization of $\dd{H_{E,x}^0(u)}/\dd{u}$ that is
...
$H_{E,x}(u)$ (see equation \eqref{eq:H_E,x}):
\begin{align}
	&  -\gamma\frac{\alpha_{\text{eff}}}{N}\frac{E}{N}\int\limits_0^\infty{\cal D}_m^0(u)\PDEV{f^0_u}{u}\dd{u}
	\nonumber \\
	&= -\frac{\alpha_{\text{eff}}}{N}\frac{N}{E}\gamma\left(\frac{E}{N}\right)^2\int\limits_0^\infty{\cal D}_m^0(u)\PDEV{f^0_u}{u}\dd{u}
	\nonumber \\
	&= \frac{\alpha_{\text{eff}}}{N}\frac{N}{E}\int\limits_0^\infty H_{E,x}\dd{u}.
\end{align}
Using a partial integration, and using the fact that $uH_{E,x}(u)$ vanishes for $u=0$ and for $u\rightarrow\infty$,
this can be written as
\begin{align}
	\ldots
	&= \frac{\alpha_{\text{eff}}}{N}\frac{N}{E}\int\limits_0^\infty H_{E,x}\PDEV{u}{u}\dd{u}.
	\nonumber \\
	&= \frac{\alpha_{\text{eff}}}{N}\frac{N}{E}\left[
		\left. \vphantom{\int\limits_0^\infty} u H_{E,x}\right|_0^\infty
		- \int\limits_0^\infty u\PDEV{H_{E,x}}{u}\dd{u}
	\right]
	\nonumber \\
...
\begin{equation}
	H_{x,\text{conv}}(u) = C_x(u)f, \quad
	H_{x,\text{diff}}(u) = - D_x(u)\DERIV{f}{u}.
	\label{eq:cd-flux-conv,diff}
\end{equation}
Among others, these individual components will be used in the analysis of the
...
This way of writing also makes the addition of fluxes les straightforward.
...
\end{equation}
where we have used the fact that $uH_{x}$ vanishes for $u=0$ and for $u\rightarrow\infty$.
Using definition \eqref{eq:cd-flux-conv,diff} to split this power contribution into
convective and diffusive parts yields
\begin{equation}
	\Theta_x
