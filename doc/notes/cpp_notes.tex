...
is the choice for temporal/spatial usually documented? Then that should be respected,
...
\begin{equation}
	...
	\label{eq:Theta_Eg}
\end{equation}
where we have used expression \eqref{eq:mueN-alt} for the electron mobility.
...
\end{align*}
The approximation of $D_eN$ and $D_uN$ is straightforward:
\begin{align}
	D_eN &\doteq \gamma \sum_k {\cal D}^0_kf_k\Delta u,
	\label{eq:DeN-disc} \\
	D_uN &\doteq \frac{\gamma}{\MEAN{u}} \sum_k u_k{\cal D}^0_kf_k\Delta u,
	\label{eq:DuN-disc}
\end{align}
...
\item This is {\em not} what is in the LoKI-B code. That is based on
	...
	\begin{itemize}
		...
		\item \SRC{ionSpatialGrowthD} provides $(\alpha_{\text{eff}}/N)^2{\cal D}_{sst}^0f^0(u)$:
			the second term on the right-hand side of equation \eqref{eq:Rsst-orig},
...
\begin{framed}
			...
			the derivative of the EEDF, instead of the more convenient expression that
...
\begin{figure}[tb]
\begin{center}
	% prescribed.pdf can be (re)created by calling 'gnuplot prescribed.gnu'
	\includegraphics[width=0.8\textwidth]{prescribed}
	\caption{The reduced prescribed probability distribution function
		$g_s(r)$ from equation \eqref{eq:prescribed_reduced}.
		For a given $\MEAN{u}$, the probability distribution function
		is obtained as $\sqrt{u}f_s(u)=g_s(u/\MEAN{u})/\MEAN{u}$, see
		equation \eqref{eq:prescribed_from_reduced}.
		The parameters $s=1$ and $s=2$ result in the Maxwellian
		and Druyvesteyn distributions, respectively.
		}
	\label{fig:prescribed}
\end{center}
\end{figure}

A reduced (dimensionless) can be obtained by scaling the energies with the mean energy, yielding
\begin{equation}
	g_s(r) := \MEAN{u}\sqrt{r\MEAN{u}}f_s(r\MEAN{u})
	...
	\label{eq:prescribed_reduced}
\end{equation}
For a given parameter value $s$ and mean energy value $\MEAN{u}$, the value
$\sqrt{u}f_s(u)$ of the probability distribution function can be obtained from
\begin{equation}
	\sqrt{u}f_s(u) := g_s(u/\MEAN{u})/\MEAN{u}.
	\label{eq:prescribed_from_reduced}
\end{equation}
The function $g_s(r)$ has been plotted in figure \ref{fig:prescribed} for a
range of shape parameter values $s$. The physically relevant cases are $s=1$
(the Maxwellian distribution) and $s=2$ (the Druyvesteyn distribution), other
$s$ values may be useful for testing purposes.
