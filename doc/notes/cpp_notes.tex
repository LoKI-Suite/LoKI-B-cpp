...
\section{Solutions for systems of the form $f^{0,1}_u(u)$}
...
These definitions allow us to write the equation as
\begin{equation}
...
NOTE that LoKI-B has an additional minus sign in the second equation to make $\mu_e$ positive,
...
\begin{framed}
	...
	\item Why should complex roots (if any) be ignored/patched? Physically
		...
		Are these in conflict with any prior assumption? Than that should be
		explained.
	\end{itemize}
\end{framed}
...
\end{equation}
This is essentially the form of the equations that is described in the
LoKI-B manual \cite[eq. 8a--b, 29--30c]{Manual_2_2_0}.  In that presentation,
the field operator is modified to also describe part of the growth effects
(the term $-\alpha_{\mbox{eff}}f^0(u)$ in the last term on the left-hand
side). This is, however, {\em not} what you find in the LoKI-B code,
where the growth terms are separated from the others. By bringing all
terms that depend on $\alpha_{\mbox{eff}}f^0(u)$ to the right-hand side
...
the resulting expressions for the fluxes $H_x(u)$ will be of the advection-diffusion
type,
\[
...
\begin{framed}
TODO: check, it seems that Loki-B docs call the convection coefficient
$c_x$ and the diffusion coefficient $d_x$ --- which is very unfortunate.
\end{framed}
...
expressions that are used in LoKI-B for the electron-electron collision term. Rockwoods expressions
have been modified for the usage in LoKI-B, BOLSIG+ and this text of $\si{eV}$ as energy unit,
which has been made clear with the suffic $\si{eV}$ on $\alpha$.
...
\begin{equation}
	...
	= \frac{e^2}{8\pi\epsilon_0^2}\gamma\frac{n_e}{N}\left(\ln\Lambda_c\right) n_e
	= 3\alpha_{eV} n_e;
	\label{eq:g_ee-alpha}
\end{equation}
...
\begin{equation}
	g_ee(u) = \frac{3\alpha_{eV}}{\gamma}\frac{n_e}{N}.
\end{equation}
...
\subsection{Discretisation of the Electron-Electron Operator}
...
The electron-electron operator $\tilde{C}^0_{ee}(u)$ is given by equations
\eqref{eq:Ctilde=-dH/du} and \eqref{eq:G*_ee}. The discrete form is obtained as
follows (see section \ref{sec:dGdu-disc} for a related discussion):
\begin{equation}
	-\left.\DERIV{H_{ee}(u)}{u}\right|_n
	\doteq
	\frac{1}{\Delta u}\left(H_{ee}(u_{\phalf{n}})-H_{ee}(u_{\mhalf{n}})\right).
\end{equation}
...
\begin{align}
	...
	H_{ee}(u_{\mhalf{n}})
		&= h_{ee}I(u_{\mhalf{n}})\frac{f_{n}+f_{n-1}}{2}
		+ h_{ee}J(u_{\mhalf{n}})\frac{f_{n}-f_{n-1}}{\Delta u}.
\end{align}
  ...
  \item In the MATLAB code, the name \SRC{alpha} is used for the factor
    ...
    \begin{enumerate}
      \item In the C++ code, the variable \SRC{alpha} has been renamed to \SRC{g_ee}
        so the code matches the documentation (note: $g_{ee}=h/\gamma$).
...
TODO:
\begin{itemize}
	\item no reference is provided for this expression.
	\item Discuss properties of the SDCS.
	\item Show that $\int_0^\infty \tilde{C}^0_{ion,i}(u)\dd{u}=\MEAN{K_{ion}}$.
\end{itemize}
...
with $K(u)$ the energy-dependent rate coefficient. Multiplying with $\sqrt{u}f^0(u)$ and integrating
