...
\end{equation}
The reason for introducing the minus sign here is that the resulting
expressions for the fluxes $H_x(u)$ will be of the advection-diffusion type,
\begin{equation}
	H_x(u) = C_x(u)f - D_x(u)\DERIV{f}{u},
	\label{eq:cd-flux}
\end{equation}
with a {\em positive} diffusion coefficient $D_x(u)$.
This choice of sign is also compatible with the field term $\dd{H_{E,g}}/\dd{u}$,
with $H_{E,g}$ as given in equation \eqref{eq:H_E,g} (which is purely diffusive).
...
\begin{equation}
	...
	\label{eq:CvsGman}
\end{equation}
In the {\em code} (matlab and the present C++ code), no tricks are done: the
terms are all brought to the {\em right-}hand side of the equation the source
...
LoKI-B supports the evaluation of swarm parameters for a prescribed EEDF. It supports
the following `generalized EEDF' (TODO: ADD REFS):
\begin{equation}
	f(u) = \left(\frac{2e}{3k_BT_e}\right)^{3/2}
