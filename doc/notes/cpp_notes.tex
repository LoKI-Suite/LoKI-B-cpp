...
\begin{equation}
	\MEAN{\DERIV{H}{u}}_P
	= \frac{B_ef_P - A_ef_E}{(\Delta u)_P} - \frac{B_wf_W - A_wf_P}{(\Delta u)_P}
...
\begin{equation}
	...
	= \frac{B_ef_P - A_ef_E}{(\Delta u)_P} - 0
...
\begin{equation}
	...
	= 0 - \frac{B_wf_W - A_wf_P}{(\Delta u)_P}
...
highest real grid cell. If we assume that the distance between the cell values of
cells $L$ and the virtual cell $E$ is $(\Delta u)_f$, the
...
\begin{align}
	\MEAN{\DERIV{H}{u}}_{P={\cal N}-1}
	&= \frac{(B_{x,e} - A_{x,e}(B_e/A_e))f_P}{(\Delta u)_P} - \frac{B_{x,w}f_W - A_{x,w}f_P}{(\Delta u)_P}
	\nonumber \\
...
\subsubsection{Relationship with the LoKI-B Coefficient Notation $H_x(u)$}
...
\end{align}
For the discrete equation for internal points we get

TODO: maybe reproduce one of the discrete equations, such as for $\bmat{M}_{E}$ in
\cite[eq. 25b]{Manual_2_2_0}.
...
\eqref{eq:disc:c(u)}, and $\dd{}/\dd{u}=\dd{}/(\Delta u)_f\dd{c})$, equation
...
the expression somewhat. We will therefore first continue under the assumption that
$\Pe\neq 0$. Then
\begin{align}
	f(u(c))
...
We consider the convection-diffusion equation in the interval $[u_L,u_H]$ and assume
that $D(u)$ is non-zero in this interval
and monotonic. We define
\begin{equation}
