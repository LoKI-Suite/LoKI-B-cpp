...
\subsection{Notation, Equation Compatibility with LoKI-B and BOLSIG+}
...
\begin{itemize}
	...
	\item In \cite[52a--b]{Manual_2_2_0} the mean velocity is called
		the mean {\em drift} velocity. That seems wrong, since the
		definition also includes the diffusion velocity in the case
	...
	\item	In BOLSIG+ and in LoKI-B, the energy is expressed in $eV$. BOLSIG+
		...
		test we systematically use $\epsilon$ for an energy in $J$, and
		$u=\epsilon/e$ for an energy in $eV$.
	\item In LoKI-B, some additional factor $\gamma$ and $1/\gamma$ appear in
...
\begin{itemize}
	\item LoKI-B: In all equations, change:
		\begin{itemize}
			...
			function, $f^0_u+f^1\cos\theta$, $f^0$, since $f$ is used for
		...
		\end{itemize}
	\item BOLSIG+: In all equations, change:
		\begin{itemize}
			\item $\mu_e\rightarrow -\mu_e$,
			\item $E\rightarrow -E$,
...
\subsection{The Two-Term Expansion of the Boltzmann Equation}
...
but we adopt the notation as follows:
With these changes, the distribution function is given by
\begin{equation}
	F_v(v,\theta,z,t) = F^0(v,z,t) + F^1(v,z,t)\cos\theta,
	\label{eq:Fv(Fv0,Fv1)}
\end{equation}
...
$u=\epsilon/e$, the energy in $eV$, and use $v=\sqrt{2eu/m}$, the energy-in-$\si{eV}$
...
\end{align}
In particular, we have the following expressions for the mean speed, the components of
mean velocity along the electric field and for the mean energy:
\begin{alignat}{2}
...
\end{alignat}
Using $F^{0,1}$ instead of $f^{0,1}$ or, equivalently, multiplying the results
with $n_e$, gives some important density and flux density expressions:
\begin{alignat}{2}
	n_u = n_e\MEAN{u} &= \int\limits_0^\infty u\sqrt{u} F^0_u(u,z,t)\dd{u} && \quad\quad\mbox{mean electron energy density} \label{eq:n_u} \\
	\Gamma_e = n_e\MEAN{v_z} &= \frac{\gamma}{3}\int\limits_0^\infty uf^1_u(u,z,t)\dd{u} && \quad\quad\mbox{electron flux density component $\parallel E$} \label{eq:Gamma_e} \\
	\Gamma_u = n_e\MEAN{uv_z} &= \frac{\gamma}{3}\int\limits_0^\infty u^2f^1_u(u,z,t)\dd{u} && \quad\quad\mbox{electron energy flux density component $\parallel E$} \label{eq:Gamma_u}
\end{alignat}
...
\subsection{Equations for $F^0_u$ and $F^1_u$}
...
\begin{equation}
	\tilde{C}^0=2\pi\gamma^3\sqrt{u}\frac{C_0}{Nn}.
	\label{eq:Ctilde0}
\end{equation}
...
\begin{equation}
	...
	\label{eq:S_e}
\end{equation}
This expression for $S_e$ will be justified later, in section \ref{sec:sources}.
...
be provided in section \ref{sec:sources}.
...
$\si{eV/m^3/s}$, yuck):
\begin{equation}
	\PDEV{n_u}{t} + \PDEV{\Gamma_u}{z} - (q_e\Gamma_e)E/e= S_u(z,t),
	\label{eq:energy-balance}
\end{equation}
This equation can be recognized as the energy balance equation for the
electrons, with the volumetric (net) power gained in by the electrons
...
Substitution of equations \eqref{eq:F=n_ef} and \eqref{eq:f0u-norm} in equations
...
\subsection{Solutions for systems of the form $f^{0,1}_u(u)$}
...
--- but $n_e$ is still allowed to depend on space or time, or both. Under these conditions,
...
relation. In the following sections we will instead discuss the cases that one of these
...
\begin{framed}
TODO/IDEA: is it useful to eliminate one the derivatives, and make some general progress,
instead of assuming already at this point that one of them is zero (next sections)?
...
\begin{align}
	\beta &:= \begin{cases}
			\displaystyle \frac{1}{\nu_e}\frac{1}{n_e}\PDEV{n_e}{t}	&: \MEAN{\nu_{eff}}=0, \\
			1							&: \mbox{otherwise},
		\end{cases}
\end{align}
so
\begin{align}
	\frac{1}{n_e}\PDEV{n_e}{t} &= \beta\MEAN{\nu_{eff}}, \\
	\MEAN{v_z}\frac{1}{n_e}\PDEV{n_e}{z} &= (1-\beta)\MEAN{\nu_{eff}}.
\end{align}
(TODO still: show that it does not matter which growth model is assumed when $\MEAN{\nu_{eff}}=0$,
so we can arbitrarily set $\beta=1$ in that case.)
What impact would that have on fluid models that
use results of the two-term expansion? Would there be significant differences?

Underlying question: when `complete` data sets are compiled using a 2-term model,
...
Following Hagelaar et al., the first term on the left-hand side of the equation for $f^0(u)$
is called defined to be $-\tilde{R}_{pt}$, and $\tilde{R}_{pt}$ called the `growth renormalization term`,
\begin{equation}
...
\begin{equation}
	- \gamma\left(\frac{E}{N}\right)^2\PDEV{}{u}\left({\cal D}_{pt}^0(u)\PDEV{f^0_u}{u}\right)
	= \tilde{C}^0(u) + \tilde{R}_{pt}(u).
\end{equation}

\subsubsection{Temporal Growth for HF Fields}
...
\begin{equation}
	\MEAN{v_z}\PDEV{n}{z}=n_e\MEAN{\nu_{\mbox{eff}}}.
\end{equation}
For $\MEAN{v_z}\neq 0$ this can be written as
\begin{equation}
	\alpha_{\mbox{eff}} := \frac{1}{n_e}\PDEV{n}{z}=\frac{\MEAN{\nu_{\mbox{eff}}}}{\MEAN{v_z}}.
	\label{eq:alpha_eff}
\end{equation}
Using \eqref{eq:alpha_eff} to eliminate the spatial derivative of $n_e$ we get
\begin{align}
	&
...
\end{align}
TODO: introduce $\mu_e$ and $D_e$ here. Or already earlier, in the general $F^{0,1}$ equations?

Combination with equation \eqref{eq:alpha_eff} gives an expression for $\alpha_{\mbox{eff}}$,
\begin{equation}
...
Note that there are two solution. We choose the one that has the desired property that
...
\begin{framed}
	{\bf Open Questions:}
	\begin{itemize}
	\item Why don't the BOLSIG+ and LoKI-B documents address the existence of a second root.
	...
	\item Why should the complex root (if any) be ignored/patched? Physically
		these would result in oscillatory $n(z)$. Is that wrong per se?
	\item Why should the second solution of the equation for $\alpha_{\mbox{eff}}$ be rejected?
	\end{itemize}
\end{framed}
...
\end{equation}
This is essentially the form of the equations that is described in the LoKI-B manual \cite[eq. 8a--b, 29--30c]{Manual_2_2_0}.
In that presentation, the field operator is modified to also describe part of the growth effects (the
term $-\alpha_{\mbox{eff}}f^0(u)$ in the last term on the left-hand side). This is, however, {\em not}
what you find in the LoKI-B code, where the growth terms are separated from the others. By bringing
all terms that depend on $\alpha_{\mbox{eff}}f^0(u)$ to the right-hand side of the equation, that
can be rewritten as
\begin{equation}
	- \gamma\left(\frac{E}{N}\right)^2\PDEV{}{u}\left(
		{\cal D}_m^0(u)\PDEV{f^0_u}{u}
	\right)
	= \tilde{C}^0(u) + \tilde{R}_{sst}(u),
\end{equation}
...
{\em TODO: find out which of these forms is actually discretized in the LoKI-B code
(after $E\rightarrow -E$ and division by $1/\gamma$). It appears to be \eqref{eq:Rsst-orig},
but I have not been able to verify the correctness of the discretisation of the points on
boundary cells.}

\subsection{Collisional Source Terms}
\label{sec:sources}

\subsubsection{Compatibility Notes}
\begin{framed}
	In the LoKI-B presentation (manual), source terms of the form $\tilde{C}_x(u) = \dd{X(u)}/\dd{u}$
	appear at the left-hand side of the equation for $f^0_u(u)$, and all terms of the final equation are divided by
	$\gamma$. As a result, in the manual they appear as
	$-\tilde{C}_x(u)/\gamma := (1/(N\gamma))\dd{G^{man}_x(u)}/\dd{u}$, with $G^{man}_x=-N X_x$.

	In the {\em code} (matlab and C++), no tricks are done and such terms discretized as
	$\tilde{C}_x(u)/\gamma = \dd{X^{code}}/\dd{u}$, with $X^{code}=X(u)/\gamma$
	(all terms are moved to the right-hand side).
\end{framed}

\subsubsection{The Elastic Source Term}

For the elastic source term $\tilde{C}_{el}$, see \cite[eq. 27]{Hagelaar2005} That gives the
expression for a single target species. By
summing we get the total elastic term. This can be written as
\begin{equation}
	\tilde{C}^0_{el}(u)=\PDEV{}{u}\left[\gamma u^2\left(\sum\limits_k x_k\frac{2m}{M_k}\sigma_{m,k}(u)\right)\left(f^0(u)+\frac{k_BT}{e}\PDEV{f^0}{u}\right)\right]
	= \PDEV{}{u}\left[\gamma u^2\sigma_{u}(u)\left(f^0(u)+\frac{k_BT}{e}\PDEV{f^0}{u}\right)\right],
	\label{eq:Ctilde0-el}
\end{equation}
where the total elastic energy transfer cross section is defined as \cite[eq. 42]{Hagelaar2005}
\begin{equation}
...
compatibility notes).
The corresponding LoKI-B flux function $G^{man}_{el}(u)$, that is reported in the manual, is equivalent to
\begin{equation}
	G^{man}_{el}(u) = -N\gamma u^2\sigma_{u}(u)\left(f^0(u)+\frac{k_BT}{e}\PDEV{f^0}{u}\right).
\end{equation}
For unknown reasons, the manual provide an expression in terms of an elastic (energy transfer) {\em frequency}.
Since $\gamma u^2=v(u)u^{3/2}$, and a frequency can be defined as $\nu_{u}(u)=N\sigma_{u}(u)v(u)$,
the result, which can be found in the manual \cite[eq. 12b]{Manual_2_2_0}, is given by
\begin{equation}
	G^{man}_{el}(u)
...
\subsubsection{The CAR Source Term}
For the CAR operator, the manual has \cite[eq. 12c]{Manual_2_2_0}
\begin{equation}
	G^{man}_{car}(u) = -\sum\limits_k 4B_k\nu_{0,k}\sqrt{u}\left[f^0(u) + \frac{k_BT_g}{e}\PDEV{f^0}{u}\right].
\end{equation}
and in terms of a cross section this can be written as
\begin{equation}
	G^{man}_{car}(u)
...
\end{equation}
As before, in the {\em code} this is brought to the right-hand side and the term is discretized as
\begin{equation}
	\tilde{C}^0(u)/\gamma = \PDEV{G^{code}_{car}}{u},
\end{equation}
with
\begin{equation}
	G^{code}_{car}(u) = 4\sigma_{car}u\left[f^0(u) + \frac{k_BT_g}{e}\PDEV{f^0}{u}\right].
\end{equation}


\subsubsection{Excitation and Deexcitation}
...
\begin{itemize}
	\item in the \SRC{oneTakesAll} case, electrons are injected
		...
		recommended by but Opal et al. \cite[fig. 4, text below tab. 1]{Opal1971}
		has $\beta=2.1$. What is the impact of these
		deviations? The manual should be more clear about that.
		Make consistent: usage of $(u,u')$ vs $(u',u)$ in equations 14 and 63.
	\item the C++ implementation of the last factor of eq. 63 from the manual seems wrong
		(added a TODO to the code).
...
so that a growth model must be used.
...
to \cite{Allis1956} (Allis, 1956) and to \cite{Ferreira2000} (Ferreira, 2000)
are provided for these expressions.
...
In \cite{Ferreira2000} work, Ferreira provides references to (again) \cite{Allis1956} and to a previous
...
\[
	...
	= \frac{e^2}{8\pi\epsilon_0^2}\frac{n_e}{N}\gamma\left(\ln\Lambda_c\right) n_e.
	\label{eq:g_ee-alpha}
\]
and will be written as
\begin{equation}
	2\nu_{ee}u^{3/2} = 3\alpha_{eV}n_e,
\end{equation}
with
\begin{equation}
	...
	\label{eq:alpha_eV}
\end{equation}
This notation is due to Rockwood \cite[below equation B1]{Rockwood1973} and is further explained
in section \ref{sec:Rockwood1973EE}, where the reader will also find a complete derivation of the
expressions that are used in LoKI-B for the electron-electron collision term.

Substitution of the expression for $\alpha_{eV}$ in equation \eqref{eq:G(u)-original} yields
\begin{equation}
	G_{ee}(u) = -3\alpha_{eV} n_e\left[I(\epsilon)f_\epsilon + J(\epsilon)\PDEV{f_\epsilon}{\epsilon}\right],
	\label{eq:G(u)}
\end{equation}
In LoKI-B, a scaling with a factor $1/(N\gamma)$ of the Boltzmann equation is used
and expressions of the form $-G_x(u)/(N\gamma)$ appear throughout. We get
\begin{equation}
	-\frac{G_{ee}(u)}{N\gamma} = g_{ee}\left[I(\epsilon)f_\epsilon + J(\epsilon)\PDEV{f_\epsilon}{\epsilon}\right],
	\label{eq:(-1/NgammaG(u)}
\end{equation}
with \cite[eqn. 37a]{Manual_2_2_0}
\begin{equation}
	g_{ee}(u) = \frac{2\nu_{ee}u^{3/2}}{N\gamma}
	= \frac{3\alpha_{eV}}{\gamma}\frac{n_e}{N}
	= \frac{n_e}{N}\frac{1}{8}\frac{e^2}{\pi\epsilon_0^2}\ln\Lambda_c.
	\label{eq:g_ee}
\end{equation}
The term in the Boltzmann equation that is associated with the electron-electron operator
is given by \cite[eqn. 38]{Manual_2_2_0},
\begin{equation}
...
\begin{itemize}
  ...
  \item The factor $g_{ee}(u)$ not used consistently. In many places we find
...
\begin{align}
	A_n &= g_{ee}\left(-\frac{I(u_{\phalf{n}})}{2\Delta u} + g_{ee}\frac{J(u_{\phalf{n}})}{(\Delta u)^2}\right),
		\label{eq:ee:A} \\
	B_n &= g_{ee}\left(+\frac{I(u_{\mhalf{n}})}{2\Delta u} + g_{ee}\frac{J(u_{\mhalf{n}})}{(\Delta u)^2}\right).
		\label{eq:ee:B}
\end{align}
Then
\begin{align}
	-\frac{1}{N\gamma}G_{ee}(u_{\mhalf{n}})\frac{1}{\Delta u}
		&= B_{n  }f_{n  } - A_{n-1}f_{n-1}, \label{eq:ee:Gbelow}  \\
	-\frac{1}{N\gamma}G_{ee}(u_{\phalf{n}})\frac{1}{\Delta u}
		&= B_{n+1}f_{n+1} - A_{n  }f_{n  }. \label{eq:ee:Gabove}
\end{align}
...
\end{equation}
An explanatory note is due here. If we evaluate expression \eqref{eq:ee:Gbelow} for $n=0$
(the first cell) or expression \eqref{eq:ee:Gabove} for $n={\cal N}-1$ (the last), each
right-hand sides involves an invalid value ($f_{-1}$ and $f_{\cal N}$, respectively). It
is to be understood that these terms are skipped. In the manual \cite[below eq 41c]{Manual_2_2_0}
this is formulated as $A_{-1}=B{\cal N}=0$, but of course those coefficients do not exist in the
arrays $\bvec{A}$ and $\bvec{B}$, and no evaluation should be attempted. Since the fluxes should
vanish on the boundaries, also the other term in each equation should be zero and we decide that
$B_0=A_{\cal N}=0$.
...
\begin{align}
	\bvec{A} &=g_{ee}\bmat{a}\bvec{f}, \label{eq:ee:A=g_eeaf} \\
	\bvec{B} &=g_{ee}\bmat{b}\bvec{f}, \label{eq:ee:B=g_eebf}
\end{align}
...
Three more modifications are made:
\begin{itemize}
  \item The last row and first column of $\bmat{A}$ are set to 0;
  \item The substitution $a_{ij} = \sqrt{a^{old}_{ij}a^{old}_{j-1,i+1}}$ is made;
  \item The matrix $\bmat{b}$ is calculated as $\bmat{a}^T$.
\end{itemize}


Rewrite a bit more to investigate the impact of the last step.
NOTE: first the harmonic averaging is done, so how relevant is this?
...
TODO: mention the reasons, fill in the details: the latter are not provided in the manual or LoKI-B paper.
The details and notation deviate from Rockwood's paper, which has most of them. Note that we use
a different EEDF than Rockwood (excluding $\sqrt{u}$ from the definition).

TODO: Most of all: show that the change of the system is not `essential', that the systems are equal
for $\Delta u \downarrow 0$.
\begin{itemize}
	...
	text of Rockwood appear to be $A$ and $B$ (below Rockwood's eq. B7) and below B8 we see the statement
	$A=B^T$.
	\begin{enumerate}
		...
		\item Show that the continuous expression for the flux is consistent with
			...
			In principle, it is enough that $a_{ji}=b_{ij}$ for $\Delta u\downarrow 0$
			for the consistency (TODO: check this, note that the number of cells and,
			consequently, the matrices, grow when the energy interval gets smaller.)
			What is the difference for a finite $\Delta u$ between these numbers?
			How does this difference affect the accuracy of the solution?
	\end{enumerate}
	...
	\begin{enumerate}
		\item How do these indices translate from MATLAB to C++? What is the meaning of index
			...
			NOTE: derive these expressions carefully for 0-based arrays, matrices.
	\end{enumerate}
	...
	\begin{enumerate}
		\item I (JvD) did not find this expression in Rockwood's paper. This requirement is
			discused in appendix B of that paper (eq. 11), which says
			\[
			...
			Of course, Rockwood's results cannot be directly compared because he uses
			the distribution function $n_e\sqrt{\epsilon}f(\epsilon)$ instead of $f(u)$.
			TODO: Is there another source for the details of the LoKI-B implementation?
			\begin{enumerate}
		...
		\item In the MATLAB code, the implementation of the application of the detailed
			...
			TODO: also fix this in the MATLAB version.

			TODO: write down Rockwood's proof in terms of zero-based C++ arrays
			and matrices for clarity. It suffices to show this for $G_{ee}(u_{\phalf{i}})$,
			I think.
	\end{enumerate}
\end{itemize}
...
\begin{itemize}
  ...
  \item In the manual \cite[eq. 39a,b,f,g]{Tejero2019}, the matrices are
	...
	TODO: the manual should be updated to reflect the reality of the code.
\end{itemize}
\end{framed}

\subsection{The Growth Terms}
\label{sec:growth-operators}

In this part of the text we provide an explanation of the growth terms (spatial
and temporal), clarify some details in \cite{Manual_2_2_0} and provide the
expressions in {\em exactly} the form in which they also appear in the C++ (and
MATLAB) codes.

In the presence of growth terms, the following things change in the EBE:
\begin{itemize}
	\item The sources on the right-hand side do not integrate to 0 (after multiplication with $\sqrt{u}$?)
	\item On the left-hand side an additional term appears that represents the
		effects of $\partial/\partial t$ (temporal growth) or $\partial/\partial z$
		(spatial growth).
	\item The calculation of $f^1(u)$ changes. In the case of temporal growth, a
		different exepression for the cross section must be used. In the
		case of spatial growth, the normal cross section is used, but a
		second term appears in $f^1(u)$ that is proportional to the growth
		(that is: to the effective Townsend coefficient $\alpha_{\mbox{eff}}$).
	\item Since $f^1(u)$ appears in the expression for the field operator,
		also that term changes. It must be calculated with a different cross
		section (temporal) or features a second term (spatial growth).
	\item The growth term depends on integral characteristics of the distribution
		function (ionization/attachment frequencies, effective townsend
		coefficient), rendering the problem non-linear: iterations will
		be needed. (The EBE may is also non-linear when electron-electron
		collisions are taken into accout.)
\end{itemize}

\subsubsection{Conservative Systems}
In the {\em absence} of non-conservative collisions (no growth), we have
\begin{align}
	&\frac{1}{N\gamma}\PDEV{G_E^c}{u} + \frac{1}{N\gamma}\sum_x\PDEV{G_x}{u}
	= S,
	\label{eq:f-2term-stationary-no-growth} \\
	&f^1(u) = -\frac{\zeta(E/N)}{\Omega_c(u)}\DERIV{f}{u}.
	\label{eq:f1-2term-stationary-no-growth}
\end{align}
Here the sum runs over all operator types except $x=E$, the field operator.
That is given by \cite[eq. 6a]{Tejero2019}
\begin{equation}
	G_E(u) = N\gamma\frac{u}{3}\frac{1}{\zeta}\frac{E}{N}f^1(u)
\end{equation}
The reason for discussing that term separately is that the field operator is affected
by the presence of growth terms, via the expression for $f^1(u)$.
Substitution of the particular expression for $f^1(u)$ for conservative systems
in this equation yields
\begin{equation}
	G_E^c(u) = -N\gamma\left(\frac{E}{N}\right)^2{\cal D}_c^0(u)\DERIV{f}{u}.
\end{equation}
Here again we have used the symbol ${\cal D}_x^0(u)$ from equation \eqref{eq:D0}.
Also note that the factors $\zeta$ cancel out in this case.

TODO: MOVE ME. The starting point is the set of equations \cite[3a--4b]{Tejero2019}, which state
the stationary form of the 2-term expansion with the growth terms included.

\subsubsection{Temporal Growth}

For temporal growth, the EBE features one additional term on the left hand side of the equation
that is linear in the effective ionization frequency and in the eedf itself:
\begin{align}
	&\frac{1}{N\gamma}\MEAN{\nu_{\mbox{eff}}}\sqrt{u}f(u)
	+\frac{1}{N\gamma}\PDEV{G_E}{u}
	+\frac{1}{N\gamma}\sum_x\PDEV{G_x}{u}
	= S,
	\label{eq:f-2term-stationary-temp} \\
	&f^1(u) = -\frac{\zeta(E/N)}{\Omega_{pt}(u)}\DERIV{f}{u}.
	\label{eq:f1-2term-stationary-temp}
\end{align}
The expression for the first anisotropy $f^1(u)$ is the same as in the conservative case,
except for the usage of $\Omega_pt(u)$ instead of $\Omega_c$. The modified cross section is
given by
(TODO: MOVE ME SOMEWHERE:)
\begin{align}
	\Omega_c   (u) &=\sigma_c(u) + \frac{1}{\gamma\sqrt{u}}\frac{\MEAN{\nu_{\mbox{eff}}}}{N}, \label{eq:Omega_c} \\
	\Omega_{pt}(u) &=\Omega_c(u) + \frac{(\omega/N)^2/(\gamma^2 u)}{\Omega_c(u)}, \label{eq:Omega_pt} \\
	\Omega_{sst}(u) &= \sigma_c(u). \label{eq:Omega_sst}
\end{align}
Substitution of this particular expression for $f^1(u)$ in the expression for $G_E(u)$ yields
\begin{equation}
	G_E^{pt}(u) = -N\gamma\left(\frac{E}{N}\right)^2{\cal D}_{pt}^0(u)\DERIV{f}{u}.
	\label{eq:G_E^pt}
\end{equation}
That modified cross section depends on the EEDF (via $\MEAN{\nu_{\mbox{eff}}}$),
so unlike in the stationary case, the term needs to be updated during the
iterative process.
\begin{itemize}
	\item One additional term must be discretized. This depends on the effective
		ionization frequency $\MEAN{\nu_{\mbox{eff}}}$, this depends itself on the
		EEDF, so iterations are required.
	\item That modified cross section depends on the EEDF (via $\MEAN{\nu_{\mbox{eff}}}$),
		so unlike in the stationary case, the term needs to be discretized
...
For spatial growth:
\begin{align}
	&\frac{1}{3}\frac{\alpha_{\mbox{eff}}}{N}uf^{1}(u)
	+\frac{1}{N\gamma}\PDEV{G_E}{u}
	+\frac{1}{N\gamma}\sum_x\PDEV{G_x}{u}
	= S,
	\label{eq:f-2term-stationary-spat} \\
	&f^1(u) =
		-\frac{\alpha_{\mbox{eff}}/N}{\Omega_{sst}(u)}f(u)
		-\frac{(E/N)}{\Omega_{sst}(u)}\DERIV{f}{u}.
	\label{eq:f1-2term-stationary-spat}
\end{align}
Substitution of this particular expression for $f^1(u)$ in the expression for $G_E(u)$ yields
\begin{alignat}{2}
	G_E^{sst}(u)
		&= -N\gamma\frac{u}{3}\frac{(\alpha_{\mbox{eff}}/N)(E/N)}{\Omega_{sst}(u)}f(u)
		&&  -N\gamma\frac{u}{3}\frac{(E/N)^2}{\Omega_{sst}(u)}\DERIV{f}{u}
		= G_E^\alpha(u) + G_E^c(u), \label{eq:G_E^sst}
\end{alignat}
which now has two terms. The latter takes the same form as before, the new one is proportional
to the effective Townsend coefficient $\alpha_{\mbox{eff}}$.

In the manual \cite[eq. 32]{Manual_2_2_0}, the field operator for the SST case (spatial growth) is
presented in this form, in which both terms are combined in a single \eqref{eq:G_E^sst}. This
is not consistent with the code, in which these two terms of the field operator are discretized
separately. The second term, which is also present for a conservative DC case is represented by
the \SRC{fieldMatrix} discretization matrix. Note that this term does not change during the
iterative process, so it needs to be set up only once after the grid has been configured or
updated. The first term of the field operator for a non-zero $\alpha_{\mbox{eff}}$ is evaluated in
\SRC{solveSpatialGrowthMatrix} (see matrix \SRC{fieldMatrixSpatialGrowth}. This is dealt with as
a separate $G_x(u)$ term (see eq. \ref{eq:-dGdu/Ngamma-discrete}); this one only has the
energy-convective term, so $c_x=0$.

Let us now consider the first term on the left-hand side of equation \eqref{eq:f-2term-stationary-spat}.
Substitution of equation \eqref{eq:f1-2term-stationary-spat} for $f^1(u)$ yields, after negating the
equation, removing the parentheses and substituting the definition \eqref{eq:D0} of ${\cal D}_{x}^0$,
\begin{equation}
	-\frac{1}{3}\frac{\alpha_{\mbox{eff}}}{N}uf^1(u)
	=
	\left(\frac{\alpha_{\mbox{eff}}}{N}\right)^2{\cal D}_{sst}^0(u)f(u)
                +\frac{\alpha_{\mbox{eff}}}{N}\frac{E}{N}{\cal D}_{sst}^0(u)\DERIV{f}{u}.
\end{equation}
The first term on the right-hand side is straightforward. It results in a diagonal term
$(\alpha_{\mbox{eff}}/N)^2{\cal D}_{sst}^0$ in the boltzmann matrix that is known in the
code as \SRC{ionSpatialGrowthD} (this is the first term on the right-hand side of the
manual eq. 29). Also the second term looks straightforward, and a central-difference scheme
...
\end{equation}
which corresponds to the second term of the manual's equation (29). The manual does not
explain how to handle the first and last cells, but for those the derivatives can of course
be approximated with the asymmetric approximations $(f_1-f_0)/\Delta$ and
$(f_{{\cal N}-1}-f_{{\cal N}-2})/\Delta u$.

Observations and questions:
\begin{itemize}
	\item It would help if the manual would follow the code; the benefit of combing the
		two terms of the field operator for spatial growth (manual 2.2.0 equation 32)
		is not clear and, as said, not on par with the code, which employs the fact
		that the original term is a linear one, so the \SRC{fieldMatrix} does not
		have to be re-evaluated.
	\item Confusion is also caused in the code by the fact that the
		helper array \SRC{g_eeSpatialGrowth} has a division by 6,
	...
	\item In the code we find something completely different for the second growth term
		(the second term on the right-hand side of the manual eq. 29),
		that I am not able to derive. It looks as if $\dd{{\cal D}(u)f(u)}/\dd{u}$
		is discretized, rather than ${\cal D}(u)\dd{f(u)}/\dd{u}$, but then still
		the handling of the boundary cells is different from what I would expect.

		My misunderstanding is probably the same is what causes my confusion
		about the evaluation of $\mu_eN$, see the following section. Make sure
		that is clarified, then the discretization of this term will also be clear.
	\item The explanation of this discretization could be added to the manual.
\end{itemize}

In BOLSIG+, another choice is made. The three terms that depend on the coefficient
$\alpha_{\mbox{eff}}$ are called the {\em growth renormalization terms}
$\tilde{R}$ and appear as source terms in the presentation \cite[eq. 17--19]{Hagelaar2005}.
\begin{align}
	&-\frac{1}{3}\frac{\alpha_{\mbox{eff}}}{N}uf^{1}(u)
	-\frac{1}{N\gamma}\PDEV{G_E}{u} \nonumber \\
	=
	&-\frac{1}{3}\frac{\alpha_{\mbox{eff}}}{N}uf^{1}(u)
	-\frac{1}{N\gamma}\PDEV{G_E^\alpha}{u}
	-\frac{1}{N\gamma}\PDEV{G_E^c}{u}
	\nonumber \\
	=
	&\frac{\alpha_{\mbox{eff}}}{N}\left[\frac{\alpha_{\mbox{eff}}}{N}{\cal D}_{sst}^0(u)f(u)
                +\frac{E}{N}{\cal D}_{sst}^0(u)\DERIV{f}{u}
	+\frac{E}{N}\DERIV{}{u}\left({\cal D}_{sst}^0(u)f(u) \right)\right]
	-\frac{1}{N\gamma}\PDEV{G_E^c}{u}
	\nonumber \\
	=
	&\frac{\alpha_{\mbox{eff}}}{N}\left[\frac{\alpha_{\mbox{eff}}}{N}{\cal D}_{sst}^0(u)f(u)
        +2\frac{E}{N}{\cal D}_{sst}^0(u)\DERIV{f}{u}
	+\frac{E}{N}f(u)\DERIV{{\cal D}_{sst}^0}{u}\right]
	-\frac{1}{N\gamma}\PDEV{G_E^c}{u}
	\nonumber \\
	=
	&\frac{\alpha_{\mbox{eff}}}{N}\left[
		{\cal D}_{sst}^0\left(
		\frac{\alpha_{\mbox{eff}}}{N}f(u)
		+2\frac{E}{N}\DERIV{f}{u}
		\right)
		+\frac{E}{N}f(u)\DERIV{{\cal D}_{sst}^0}{u}
	\right]
	-\frac{1}{N\gamma}\PDEV{G_E^c}{u}
	\nonumber \\
	=
	&\gamma^{-1}R(u) - \frac{1}{N\gamma}\PDEV{G_E^c}{u}.
\end{align}
The $R(u)$ in this expression is that from \cite[eq. 19]{Hagelaar2005}.
For the purpose of discretization, it seems to make more sense to bundle the two
terms that are proportional to $f(u)$, resulting in
\begin{equation}
	-\frac{1}{3}\frac{\alpha_{\mbox{eff}}}{N}uf^{1}(u)
	-\frac{1}{N\gamma}\PDEV{G_E}{u}
	=
	\frac{\alpha_{\mbox{eff}}}{N}\left[
		\left(
		\frac{\alpha_{\mbox{eff}}}{N}
		{\cal D}_{sst}^0
		+\frac{E}{N}\DERIV{{\cal D}_{sst}^0}{u}
		\right)f(u)
		+2\frac{E}{N}{\cal D}_{sst}^0\DERIV{f}{u}
	\right]
	-\frac{1}{N\gamma}\PDEV{G_E^c}{u}.
\end{equation}

\subsection{The evaluation of $D_eN$ and $\mu_eN$}

The expressions for $D_eN$ and $\mu_eN$ are given in \cite[eq. 48a--b]{Manual_2_2_0}.
Using definition \eqref{eq:D0} of ${\cal D}^0(u)$, which is also used in the code for
the evaluation of $D_e$ and $\mu_eN$, these can be written as
\begin{align}
	D_eN   = \gamma\int\limits_0^\infty {\cal D}_x^0(u)f(u)\dd{u}, \label{eq:DeN} \\
	\mu_eN = \gamma\int\limits_0^\infty {\cal D}_x^0(u)\DERIV{f}{u}\dd{u}. \label{eq:mueN}
\end{align}
TODO: LoKI-B has a minus sign in the second equation to make $\mu_e$ positive.

In the code, the drift velocity $v_d=-\mu_eE=-(\mu_eN) \times (E/N)$ is evaluated.
The code defines four arrays on the cell domain ($0\leq k<{\cal N}$)
(in \SRC{solveSpatialGrowthMatrix}):
\begin{align}
...
and replace ${\cal D}^0_{{\cal N}-2}/(2\Delta u)$ with $({\cal D}^0_{{\cal N}-1}-{\cal D}^0_{{\cal N}-2})/\Delta u$.
(These fixes can be translated back in modifications of ${\cal U}^-$ and ${\cal U}^+$, of course.
That would also fix the usage of the coefficient ${\cal U}^\pm$ in the assembly of the discretization matrix.)
...
\section{Notes on Rockwood's 1973 Paper}

\label{sec:Rockwood1973Notes}

\subsection{Nomenclature, Variables, Relation with LoKI-B}
...
\subsubsection{Transforming the high-energy integrals}
...
\subsubsection{Transforming the low-energy integrals}
...
\section{Derivation of the Klein-Rosseland Relation}
...
\section{Derivation of the Ionization Operators}
...
\begin{align}
	...
	\\
	\delta(u/a-u_0) &=|a|\delta(u-au_0).
\end{align}
\begin{framed}
	In the text below we use, a number of times, the assumption that
	\[
		\int\limits_{u_1}^{u_2} f(u)\delta(u-u_1)\dd{u} = f(u_1),
	\]
	in other words: that the peak is fully enclosed while it is at the boundary.
	This is mathematical nonsense. Some authors assume a factor $1/2$ there,
	but this cannot be relied on. It is almost as if the lower boundary is
	defined as $u_1^-$, which is symbolic notation for the voodoo
	\[
...
Single Differential Cross Section $\sigma_{ion,i}^{sec}(u,u')$,
\begin{equation}
	\tilde{C}^0_{ion,i}(u)
	=\gamma\delta_i\left[
...
\subsection{The One-Takes-All Model}
...
integral undefined, but the result in LoKI-B \cite[eq. 15, 1st term]{Manual_2_2_0}
and BOLSIG+ \cite[eq. 30, 2nd term]{Hagelaar2005} is obtained by assuming that the peak is
fully enclosed in the domain of integration,
\begin{equation}
	\int\limits_{u+V_{ion,i}}^{2u+V_{ion,i}}u'\sigma_{ion,i}(u')\delta(u'-V_{ion,i}-u)f^0(u')\dd{u'}
	=
	(u+V_{ion,i})\sigma_{ion,i}(u+V_{ion,i})f^0(u+V_{ion,i}).
...
\eqref{eq:del(u-u0)f^0(u)=del(u-u0)f^0(u0)} allows us to set $u=0$ there, the result is
\begin{equation}
	\int\limits_{2u+V_{ion,i}}^{\infty}u'\sigma_{ion,i}(u')\delta(u)f^0(u')\dd{u'}
	= \delta(u)\int\limits_{V_{ion,i}}^{\infty}u'\sigma_{ion,i}(u')f^0(u')\dd{u'}
	= \delta(u)\MEAN{K_{ion,i}},
\end{equation}
where we have $\MEAN{K_{ion,i}}$ the ionization rate coefficient for state $i$ as
\[
	\MEAN{K_{ion,i}} = \gamma\int\limits_{V_{ion,i}}^{\infty}u'\sigma_{ion,i}(u')f^0(u')\dd{u'}.
\]
...
that is the result that you find in \cite[eq. 30, last term]{Hagelaar2005}.
...
\begin{align*}
	...
	.
\end{align*}
Again the peak as located at the boundary of the integration domain, in the last step
we have assumed that it is fully enclosed. For the second term we find, along the same lines,
the identical result, but also here we rely on the peak being {\em enclosed} at the boundary,
which is mathematically dubious,
\begin{align*}
	&\gamma\delta_i
...
\[
	...
	\right].
\]
Integration over all energies yields,
\begin{align}
...
\end{align}
which is the expected result. In the first integral we made the substitution $t=2u+V_{ion,i}$, and note that
(in the last step) the lower boundary can be either 0 or $V_{ion,i}$ since $\sigma_{ion,i}(u)=0$
for $u'<V_{ion,i}$. Also note that $\gamma u\sigma(u)f^0(u)=\sigma(u)v(u)\sqrt{u}f^0(u):=K(u)\sqrt{u}f^0(u)$,
with $K(u)$ the energy-dependent rate coefficient. Muliplying with $\sqrt{u}f^0(u)$ and integrating
over $u$ yields its average value.

\begin{framed}
NOTE that the BOLSIG+ expression \cite[eq. 29]{Hagelaar2005} appears to be wrong: with the factor 2 instead of
4 this integral will be zero instead.
\end{framed}
