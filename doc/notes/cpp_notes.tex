...
\end{equation}
with a {\em positive} diffusion coefficient $h_x(u)c_x(u)$.
...
This notation follows the LoKI-B docs, which calls the convection coefficient
$d_x$ and the diffusion coefficient $c_x$, instead of the other way around.
For later purposes we will adopt a more standard notation for the flux,
\begin{equation}
	H_x(u) = C_x(u)f - D_x(u)\DERIV{f}{u},
	\label{eq:cd-flux}
\end{equation}
with convection coefficient $C_x(u)$ and diffusion coefficient $D(u)$. Comparison
with \eqref{eq:H_x(h_x,c_x,d_x)} reveals that
\begin{align}
...
\subsection{Expressions for the Total Flux $H(u)$}
...
\begin{equation}
	\MEAN{\DERIV{H}{u}}_P
	= (B_ef_P - A_ef_E) - (B_wf_W - A_wf_P)
	= -B_wf_W + (A_w+B_e)f_P - A_ef_E
...
\subsection{Expression for Flux Contributions $H_x(u)$}
...
\begin{align}
	\MEAN{\DERIV{H}{u}}_{P={\cal N}-1}
	&= (B_{x,e} - A_{x,e}(B_e/A_e))f_P - (B_{x,w}f_W - A_{x,w}f_P)
	\nonumber \\
	&=  - B_{x,w}f_W + (A_{x,w} + B_{x,e} - A_{x,e}(B_e/A_e) )f_P
	\nonumber \\
	&= [\mat{M}_{cd}\bvec{f}]_{P={\cal N}-1}.
...
ters of the adjacent cell values using a first-order Taylor expansion. These
values are then independent of the values of the coefficients $C$ and $D$ around
that face. Firstly, we have
\begin{equation}
...
\end{equation}
Substitution in the expression for the flux density yields
\begin{align}
...
\begin{align}
	A_f &= \left(\frac{D_f}{(\Delta u)_f}-C_fc_f    \right), \label{eq:disc:A_f-cd(C,D)} \\
	B_f &= \left(\frac{D_f}{(\Delta u)_f}+C_f(1-c_f)\right). \label{eq:disc:B_f-cd(C,D)}
\end{align}

\subsubsection{Expression for Flux Contributions $H_x(u)$}
...
TODO: specialize for $c_f=\frac{1}{2}$ and show that the LoKI-B (manual,paper)
expressions are obtained when using $h_x$, $c_x$ and $d_x$ (covered also
elsewhere in this text, except for the handling of the (upper) boundary face).
...
equations \eqref{eq:be:dHdu=S} and \eqref{eq:cd-flux-total} for $f(u)$
...
\eqref{eq:disc:cd-flux-total} can be written as
\begin{equation}
...
$\sum_xD_x=D$ that are part of equation \eqref{eq:disc:cd-flux-total} results in equation
...
\section{Old version: Discretization of the Flux-Divergence Terms}

TODO: merge some material from this section into the preceding section,
most stuff here hasbecome obsolete.

The flux functions $H_x(u)$ that appear in LoKI-B are of the advection-diffusion
...
previous expressions. Substitution of these flux approximations in equation \eqref{eq:mean-dH/du_P} results in
\begin{equation}
  \MEAN{\DERIV{H_x}{u}}_k
...
the coefficients as
\begin{align*}
    {\cal B}_{x,\phalf{k}} &= \frac{h_x(u_{\phalf{k}})c_x}{\Delta u}\left(1-\frac{\Pe_{\phalf{k}}}{2}\right), \\
    {\cal A}_{x,\phalf{k}} &= \frac{h_x(u_{\phalf{k}})c_x}{\Delta u}\left(1+\frac{\Pe_{\phalf{k}}}{2}\right),
\end{align*}
where the {\em grid P\'eclet number} $\Pe$ has been introduced,
\begin{equation}
	\Pe_{\phalf{k}} = \frac{d_x\Delta u}{c_x}.
\end{equation}
...
\end{equation}
Integration of the balance equation \eqref{eq:disc:be} over the interval $[u_f,u]$ gives
\begin{equation}
	H(u) - H_f
