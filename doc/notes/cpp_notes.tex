...
\begin{equation}
	-\DERIV{G_{ee}(u)}{u} = g_{ee}\left[I(\epsilon)f_\epsilon + J(\epsilon)\PDEV{f_\epsilon}{\epsilon}\right],
...
\begin{equation}
	...
	= \frac{1}{8}\frac{e^2}{\pi\epsilon_0^2}\ln\Lambda_c.
	\label{eq:g_ee}
\end{equation}
...
\begin{itemize}
  ...
  \item TODO: The choice for the Debye lenght (without ion shielding) should be
...
\begin{align}
	...
	b_{ij} &= \begin{cases}
			+\frac{1}{2}\sqrt{u_j}          + \frac{2}{3}\frac{u_j^{3/2}}{\Delta u}	    & j \leq i-1 \\
			\phantom{\frac{1}{2}\sqrt{u_j}} + \frac{2}{3}\frac{u_{\mhalf{i}}^{3/2}}{\Delta u} & j > i-1
		\end{cases}. \label{eq:ee:b}
\end{align}
TODO: explain the non-existing elements ($A_{-1}$ and $B_{\cal N}$). Assumed zero? And how is this
related to the matrix modifications below?
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item In the MATLAB code, the implementation of the application of the detailed
			...
			is still 0 until this bug and solution have been confirmed. Local testing
			shows that (only) after this fix, the flux $G_{ee}(u)$ vanishes up to
			machine accuracy for a Maxwellian EEDF.
...
\begin{equation}
	...
	+\frac{1}{N\gamma}\sum_x\PDEV{}{u}\left(\frac{J_x}{n_e}\right)
	= S,
...
\end{align}
and $\alpha$ is given below equation R.B1,
\begin{equation}
...
Substitution of these results equation \eqref{eq:rockwood:J_ee} yields
\begin{equation}
  J_{ee} = -3\alpha n_e^2\left[I(\epsilon)f_\epsilon + J(\epsilon)\PDEV{f_\epsilon}{\epsilon}\right], \\
\end{equation}

The last step is to cater to LoKI's usage of $\si{eV}$ as unit. To this end we again
use $\epsilon = u(\epsilon)e$ and $f_\epsilon(\epsilon)\sqrt{\epsilon}\dd{\epsilon}
= f_u(u)\sqrt{u}\dd{u}$, from which it follows that $f_\epsilon(\epsilon) = f_u(u)e^{-3/2}$.
...
\eqref{eq:rockwood:G(J)} gives equation \eqref{eq:G(u)}, as it should.
