...
\begin{itemize}
	...
	\item We use $F$ for distribution functions that are normalized to the
		...
		normalized distribution functions. This is the same is LoKI-B,
	...
	\item The angle $\theta$ is defined to be the angle with the positive
		...
		to be aligned with this axis, so $\vec{E}(t)=E(t)\vec{e}_z$.
		This is the convention that is also used in \cite{Hagelaar2005}:
		see the discussion between equations 3 and 4 of that paper.

		In LoKI-B, the electric field $E(t)$ is defined with an explicit
		minus sign (see e.g. \cite[eq. 1]{Manual_2_2_0}), which states
		$E(t)=-E_p\cos(\omega t)\vec{e}_z$, although I could not
		find these minus signs back in the code. It appears that in the
		minus sign has been obsorbed in the statement of the equations,
		instead, as if the quantity $E(t)$ is defined as
		$E(t)=-\vec{E}(t)\cdot\vec{e}_z$.
	...
	\item The Townsend coefficient has been defined as
		...
		explicit minus signs. Also $\alpha$ will be negative in case
		the growth happens in the negative $z$-direction (which normally
		happens if the electric field points in the positive direction).

		Also LoKI-B defines $\alpha$ this way, see \cite[eq. 52b]{Manual_2_2_0}.
		However, because of the explicit minus sign of the electric field,
		$\alpha$ will be be positive when $E$ is positive (assuming the
		drift velocity is not countered by an opposing diffusion velocity
		that is larger in magnitude).

		In BOLSIG+, we see $\alpha=-(\dd{n_e}/\dd{z})/n_e$ (equation
	...
	\item In \cite[52a--b]{Manual_2_2_0} the mean velocity is called
		the mean {\em drift} velocity. I consider that a bug, since the
		definition also includes the diffusion velocity --- in the
		case of the PT model the latter happens to be zero, but it is
		present in eq. 52b for the spatial growth case. In this document
		the mean velocity is just called $\MEAN{v_z}$.
...
\begin{itemize}
	\item LoKI-B: In all equations, change:
		\begin{itemize}
			...
			\item $f \rightarrow f^{0}_u$. There is no symbol
			for $f^0_u+f^1\cos\theta$, $f^0$ is used for $f^0_u$.
			I consider this very confusing and hope it can be changed in
			the LoKI-docs. I will make this change in the C++ code.
		\end{itemize}
...
\end{equation}
From the previous two expresssions it follows that the function has the integral properties
\begin{align}
	&\int\limits_0^\infty F^0_\epsilon(\epsilon,z,t)\sqrt{\epsilon}\dd{\epsilon}=n_e(z,t),
	\label{eq:F0eps-norm} \\
	&\int\limits_0^\infty F^1_\epsilon(\epsilon,z,t)\sqrt{\epsilon}\dd{\epsilon}=0.
	\label{eq:F1eps-int}
\end{align}
...
\begin{framed}
 \noindent Note that the only change from \eqref{eq:F^{0,1}_eps(eps)} to
  ...
  dissipation: $-\Gamma E \rightarrow q_e\Gamma E$ to convert $eV/m^3/s$ into
  $W/m^3$.
\end{framed}
...
\begin{align}
	...
	\label{eq:F0u-norm} \\
	&\int\limits_0^\infty F^1_u(u,z,t)\sqrt{u}\dd{u}=0.
	\label{eq:F1u-int}
\end{align}
...
\end{align}
Here we have used that fact that $u^{p+q}F^{0,1}(u)$ vanishes at $u=0$ for $p+q\geq 0$,
and goes to for $u\rightarrow\infty$. As an example,
...
With the notational changes that have been indicated in the beginning of this
section, by taking $E$ outside of the $u$-differentation and after multiplication
...
\begin{equation}
	S_e(z,t)
	= 2\pi\gamma^3\int\limits_0^\infty\sqrt{u}C^0(u)\dd{u}
...
\begin{equation}
	S_u(z,t) = 2\pi\gamma^3\int\limits_0^\infty u\sqrt{u}C^0(u)\dd{u}.
	\label{eq:S_u}
\end{equation}
...
\begin{align}
	...
	+ \frac{\gamma}{3}u\PDEV{f^1_u}{z}
	- \frac{\gamma}{3}E\PDEV{u f^1}{u}
	= 2\pi\gamma^3\sqrt{u}\frac{C^0(u)}{n_e},
	\label{eq:ebe-2term-f-u0} \\
...
the source term $C^0$ to be functions of space and time coordinates.

\subsection{Solutions for systems of the form $f^{0,1}_u(u)$}

At the basis of a number of interesting solutions is the simplification of
\eqref{eq:F=n_ef} to
\begin{equation}
...
The {\em Temporal Growth} model assumes, in addition, that the system is
uniform, $\partial/\partial z=0$, yielding
\begin{align}
	&\sqrt{u}f^0_u\frac{1}{n_e}\PDEV{n_e}{t}
	- \frac{\gamma}{3}E\PDEV{u f^1}{u}
	= 2\pi\gamma^3\sqrt{u}\frac{C^0(u)}{n_e},
...
tell us that $\dd{n_e}/\dd{t}=n_e\MEAN{\nu_{\mbox{eff}}}$, resulting in
\begin{align}
	&\sqrt{u}f^0_u\MEAN{\nu_{\mbox{eff}}}
	- \frac{\gamma}{3}E\PDEV{u f^1}{u}
	= 2\pi\gamma^3\sqrt{u}\frac{C^0(u)}{n_e},
	\label{eq:ebe-2term-f-u0(u)-temp} \\
	&f^1_u(u)
	= \frac{E}{N}\frac{1}{\Omega_c(u)}\PDEV{f^0_u}{u},
	\label{eq:ebe-2term-f-u1(u)-temp} \\
	&\Omega_c := \sigma_m(u) + \MEAN{\nu_{\mbox{eff}}}/(N\gamma\sqrt{u}).
	\label{eq:OmegaPT}
\end{align}
Substitution of the second equation in the first yields an equation for $f^0_u(u)$,
\begin{equation}
	\sqrt{u}f^0_u\MEAN{\nu_{\mbox{eff}}}
	- N\gamma\left(\frac{E}{N}\right)^2\PDEV{}{u}\left(\frac{u}{3\Omega_c(u)}\PDEV{f^0_u}{u}\right) = 2\pi\gamma^3\sqrt{u}\frac{C^0(u)}{n_e}.
\end{equation}
After dividing by $N$ and using $K_{\mbox{eff}}$ that has been defined in equation \eqref{eq:S_e(K^eff)},
this becomes
\begin{equation}
	\sqrt{u}f^0_uK_{\mbox{eff}}
	- \gamma\left(\frac{E}{N}\right)^2\PDEV{}{u}\left(\frac{u}{3\Omega_c(u)}\PDEV{f^0_u}{u}\right) = 2\pi\gamma^3\sqrt{u}\frac{C^0(u)}{n_eN}.
\end{equation}
{\bf NOTE: } We reapeat one more time that the corresponding LoKI-B expressions \cite[eq. 7a with 12a substituted, and 7b]{Manual_2_2_0}
are obtained by replacing $E\rightarrow -E$.

\subsubsection{Spatial Growth}
The {\em Spatial Growth} model allows spatial variations of $n_e$, but assumes
that the system is {\em stationary}, $\partial/\partial t=0$, yielding
\begin{align}
	&
	  \frac{\gamma}{3}uf^1_u\frac{1}{n_e}\PDEV{n_e}{z}
	- \frac{\gamma}{3}E\PDEV{u f^1}{u}
...
\end{align}
By multiplying the last equation with $\gamma u/3$ and integrating over all energies
we obtain
\begin{align}
	&
	\MEAN{v_z} = \frac{\gamma}{3}\int\limits_0^\infty uf^1_u\dd{u}
	=
		  E           \frac{\gamma}{N}\int\limits_0^\infty \frac{u}{3\sigma_m(u)}\PDEV{f^0_u}{u}\dd{u}
		- \alpha_{\mbox{eff}}\frac{\gamma}{N}\int\limits_0^\infty \frac{u}{3\sigma_m(u)}f^0_u\dd{u}
...
\end{equation}
which has real-valued roots if $(\mu_eE)^2-4D_e\MEAN{\nu_{\mbox{eff}}}>0$. When simplifying,
note that normally $\mu$ is negative, $E$ could be positive or negative, and $\alpha$ has the
same sign as the product $\mu_eE$. It is zero when the discriminant is zero, or may be complex.
Note that there is a second solution. We choose the one that has the desired property that
$\alpha_{\mbox{eff}}=0$ when $\MEAN{\nu_{\mbox{eff}}}=0$.

\begin{framed}
	{\bf Open Questions:}
	\begin{itemize}
	\item What is the physical significance of $\MEAN{v_z}$ is
		0, in case the previous defintion of $\alpha_{\mbox{eff}}$ fails?
		(And what are the implications?)
	\item Why should the imaginary root (if any) be ignored/patched? Physically
		these would result to oscillatory $n(z)$. Is thet wrong per se?
	\item For $v_z=0$ we define $\alpha_{\mbox{eff}}=0$. TODO: is that true? We
...
\begin{align}
	...
	&f^1(u) = -\frac{\zeta(E/N)}{\Omega_{c}(u)}\DERIV{f}{u}.
...
\begin{equation}
	G_E^c(u) = -N\gamma\left(\frac{E}{N}\right)^2{\cal D}_{c}^0(u)\DERIV{f}{u}.
\end{equation}
Following the code (but not the manual!), we have here introduced the symbol
${\cal D}_x^0(u)$,
\begin{equation}
...
\end{align}
The $R(u)$ is this expression is that from \cite[eq. 17]{Hagelaar2005}.
